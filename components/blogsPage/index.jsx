...
export default function BlogPage() {
  ...
  // Sort blog posts by date descending (most recent first)
        ...
        {filteredPosts.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <Link
              ...
              className="group text-gray-900 no-underline hover:underline"
            >
            ...
            <div className="flex flex-col justify-center gap-4">
              {filteredPosts.slice(1, 4).map((post) => (
                <Link
                  key={post.slug}
                  href={`/blog/${post.slug}`}
                  className="flex items-center gap-4 group text-gray-900 no-underline hover:underline"
                >
                  <Image
                    src={post.image}
                    alt={post.title}
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {filteredPosts.slice(4).map((post) => (
                <Link
                  key={post.slug}
                  href={`/blog/${post.slug}`}
                  className="flex flex-col max-w-sm items-center p-2 rounded-lg gap-4 group bg-white shadow-md text-gray-900 no-underline hover:underline"
                >
                  <Image
                    src={post.image}
                    alt={post.title}
