...
const Navbar = () => {
  ...
  return (
    <nav id='navbar' className={`flex justify-between items-center flex-col flex-lg-row ${(scrolled || showMenu) ? 'bg-black' : ''}`}>
      ...
      <div className={`nav-links mx-10 ${showMenu ? 'openMenu' : 'closedMenu'}`}>
        <ul id="main-menu" className="nav-menu flex m-0 items-center gap-8">
          ...
          <li className={`nav-item ${pathname === '/our-services' ?  'active': ''}`}>
            <Link href="/our-services" className="nav-link" onClick={() => setShowMenu(false)}>Our Services</Link>
          </li>
